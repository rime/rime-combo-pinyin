...
輸入方案 `combo_pinyin` 可通過修改配置選擇以下 [並擊鍵盤佈局](layouts.md)：
...
七指禪佈局支持自定義縮略碼與速記方案，利用閒置的並擊組合快速輸入常用詞。

縮略碼不遵循嚴格的編碼規則，需要使用者根據自己的用詞習慣和對拼音的理解逐一制定。
將詞語中標誌性的語音信息壓縮在一次並擊的編碼中以方便記憶，但不得與已有並擊碼衝突。

擴展速記方案與縮略碼詞庫可參考：[℞ `lotem/rime-combo-pinyin-steno`](https://github.com/lotem/rime-combo-pinyin-steno)。
